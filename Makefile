...
build: docker/Dockerfile docker/docker-compose.yml
	docker-compose -f ./docker/docker-compose.yml build myreco

test:
	docker-compose -f ./docker/docker-compose.yml run -e TOXENV=${TOX} myreco run-tests
